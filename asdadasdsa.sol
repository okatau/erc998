...
interface IKOAccessControlsLookup {
    ...
    function isVerifiedArtistProxy(address _artist, address _proxy)
        external
        view
        returns (bool);
    ...
    function hasContractOrAdminRole(address _address)
        external
        view
        returns (bool);
}
...
interface ITokenUriResolver {
    ...
    function tokenURI(uint256 _editionId, uint256 _tokenId)
        external
        view
        returns (string memory);

    /// @notice Do we have an edition level or token level token URI resolver set
    function isDefined(uint256 _editionId, uint256 _tokenId)
        external
        view
        returns (bool);
}
    ...
    IHasSecondarySaleFees // Rariable / Foundation royalties
{
    ...
    function getSizeOfEdition(uint256 _editionId)
        external
        view
        returns (uint256 _size);

    function getEditionSizeOfToken(uint256 _tokenId)
        external
        view
        returns (uint256 _size);
    ...
    function isEditionSalesDisabled(uint256 _editionId)
        external
        view
        returns (bool);

    // Has the edition been disabled / soft burnt OR sold out
    function isSalesDisabledOrSoldOut(uint256 _editionId)
        external
        view
        returns (bool);

    // Work out the max token ID for an edition ID
    function maxTokenIdOfEdition(uint256 _editionId)
        external
        view
    ...
    function getReverseAvailablePrimarySaleToken(uint256 _editionId)
        external
        view
        returns (uint256 _tokenId);
    ...
    function hasMadePrimarySale(uint256 _editionId)
        external
        view
        returns (bool);
    ...
    function getEditionIdOfToken(uint256 _tokenId)
        external
        pure
    ...
    function hadPrimarySaleOfToken(uint256 _tokenId)
        external
        view
        returns (bool);
}
...
abstract contract TopDownERC20Composable is
    ERC998ERC20TopDown,
    ...
    Context
{
        ...
        returns (uint256)
    {
        ...
        uint256 editionBalance = editionTokenERC20Balances[editionId][
            _erc20Contract
        ];
        uint256 tokenEditionBalance = editionBalance /
            koda.getSizeOfEdition(editionId);
        uint256 spentTokens = editionTokenERC20TransferAmounts[editionId][
            _erc20Contract
        ][_tokenId];
    ...
    function getERC20(
        ...
        uint256 editionId = IKODAV3(address(this)).getEditionIdOfToken(
            _tokenId
        );
        bool editionAlreadyContainsERC20 = ERC20sEmbeddedInEdition[editionId]
            .contains(_erc20Contract);
        bool nftAlreadyContainsERC20 = ERC20sEmbeddedInNft[_tokenId].contains(
            _erc20Contract
        );
    ...
    function _composeERC20IntoEdition(
        address _from,
        uint256 _editionId,
        address _erc20Contract,
        uint256 _value
        ...
        editionTokenERC20Balances[_editionId][_erc20Contract] =
            editionTokenERC20Balances[_editionId][_erc20Contract] +
            _value;
        ...
        returns (uint256)
    {
        ...
        return
            ERC20sEmbeddedInNft[_tokenId].length() +
            ERC20sEmbeddedInEdition[editionId].length();
    }
        ...
        returns (address)
    {
        ...
        if (_index >= numOfERC20sInNFT) {
            ...
            return
                ERC20sEmbeddedInEdition[editionId].at(
                    _index - numOfERC20sInNFT
                );
        }
    ...
    function _prepareERC20LikeTransfer(
        uint256 _tokenId,
        address _to,
        address _erc20Contract,
        uint256 _value
        ...
        bool nftContainsERC20 = ERC20sEmbeddedInNft[_tokenId].contains(
            _erc20Contract
        );

        IKODAV3 koda = IKODAV3(address(this));
        uint256 editionId = koda.getEditionIdOfToken(_tokenId);
        bool editionContainsERC20 = ERC20sEmbeddedInEdition[editionId].contains(
            _erc20Contract
        );
        ...
        require(
            balanceOfERC20(_tokenId, _erc20Contract) >= _value,
            "Exceeds balance"
        );

        uint256 editionSize = koda.getSizeOfEdition(editionId);
        uint256 tokenInitialBalance = editionTokenERC20Balances[editionId][
            _erc20Contract
        ] / editionSize;
        uint256 spentTokens = editionTokenERC20TransferAmounts[editionId][
            _erc20Contract
        ...
        if (editionTokenBalance >= _value) {
            editionTokenERC20TransferAmounts[editionId][_erc20Contract][
                _tokenId
            ] = spentTokens + _value;
            ...
            editionTokenERC20TransferAmounts[editionId][_erc20Contract][
                _tokenId
            ] = spentTokens + editionTokenBalance;
            uint256 amountOfTokensToSpendFromTokenBalance = _value -
                editionTokenBalance;
            ERC20Balances[_tokenId][_erc20Contract] =
                ERC20Balances[_tokenId][_erc20Contract] -
                amountOfTokensToSpendFromTokenBalance;
        }
        ...
        if (editionContainsERC20) {
            ...
            for (uint256 i = 0; i < editionSize; i++) {
                uint256 tokenBal = tokenInitialBalance -
                    editionTokenERC20TransferAmounts[editionId][_erc20Contract][
                        editionId + i
                    ];
...
abstract contract TopDownSimpleERC721Composable is Context {
    ...
    mapping(address => mapping(uint256 => uint256))
        public composedNFTsToKodaToken;
    ...
    function composeNFTsIntoKodaTokens(
        uint256[] calldata _kodaTokenIds,
        address _nft,
        uint256[] calldata _nftTokenIds
    ) external {
        uint256 totalKodaTokens = _kodaTokenIds.length;
        require(
            totalKodaTokens > 0 && totalKodaTokens == _nftTokenIds.length,
            "Invalid list"
        );

        IERC721 nftContract = IERC721(_nft);

        for (uint256 i = 0; i < totalKodaTokens; i++) {
            uint256 _kodaTokenId = _kodaTokenIds[i];
            uint256 _nftTokenId = _nftTokenIds[i];

            require(
                IERC721(address(this)).ownerOf(_kodaTokenId) ==
                    nftContract.ownerOf(_nftTokenId),
                "Owner mismatch"
            );
...
contract Konstants {
    ...
    function _editionFromTokenId(uint256 _tokenId)
        internal
        pure
        returns (uint256)
    {
...
abstract contract BaseKoda is Konstants, Context, IKODAV3 {
    ...
    event AdminEditionReported(
        uint256 indexed _editionId,
        bool indexed _reported
    );
    event AdminArtistAccountReported(
        address indexed _account,
        bool indexed _reported
    );
    event AdminUpdateAccessControls(
        IKOAccessControlsLookup indexed _oldAddress,
        IKOAccessControlsLookup indexed _newAddress
    );
    ...
    function reportEditionId(uint256 _editionId, bool _reported)
        public
        onlyAdmin
    {
    ...
    function reportArtistAccount(address _account, bool _reported)
        public
        onlyAdmin
    {
    ...
    function updateBasisPointsModulo(uint256 _basisPointsModulo)
        public
        onlyAdmin
    {
    ...
    function updateSecondaryRoyalty(uint256 _secondarySaleRoyalty)
        public
        onlyAdmin
    {
    ...
    function updateAccessControls(IKOAccessControlsLookup _accessControls)
        public
        onlyAdmin
    {
    ...
    IKODAV3Minter
{
    ...
    event EditionSalesDisabledToggled(
        uint256 indexed _editionId,
        bool _oldValue,
        bool _newValue
    );
    ...
    event AdminRoyaltiesRegistryProxySet(
        address indexed _royaltiesRegistryProxy
    );
    ...
    modifier validateCreator(uint256 _editionId) {
        ...
        require(
            _msgSender() == creator ||
                accessControls.isVerifiedArtistProxy(creator, _msgSender()),
    ...
    function mintBatchEdition(
        ...
        address _to,
        string calldata _uri
    ) public override onlyContract returns (uint256 _editionId) {
    ...
    function mintBatchEditionAndComposeERC20s(
        ...
        address[] calldata _erc20s,
        uint256[] calldata _amounts
    ) external override onlyContract returns (uint256 _editionId) {
        uint256 totalErc20s = _erc20s.length;
        require(
            totalErc20s > 0 && totalErc20s == _amounts.length,
            "Tokens invalid"
        );

        _editionId = _mintBatchEdition(_editionSize, _to, _uri);

        for (uint256 i = 0; i < totalErc20s; i++) {
            _composeERC20IntoEdition(_to, _editionId, _erc20s[i], _amounts[i]);
        }
    }

    function _mintBatchEdition(
        uint16 _editionSize,
        address _to,
        string calldata _uri
    ) internal returns (uint256) {
        require(
            _editionSize > 0 && _editionSize <= MAX_EDITION_SIZE,
            "Invalid size"
        );
    ...
    function editionAdditionalMetaData(uint256 _editionId)
        public
        view
        returns (string memory)
    {
    ...
    function tokenAdditionalMetaData(uint256 _tokenId)
        public
        view
        returns (string memory)
    {
    ...
    function editionAdditionalMetaDataForToken(uint256 _tokenId)
        public
        view
        returns (string memory)
    {
        ...
        validateEdition(_editionId)
    {
        ...
        emit EditionSalesDisabledToggled(
            _editionId,
            editionSalesDisabled[_editionId],
            !editionSalesDisabled[_editionId]
        );
    ...
    function _getCreatorOfEdition(uint256 _editionId)
        internal
        view
        returns (address _originalCreator)
    {
    ...
    function _maxTokenIdOfEdition(uint256 _editionId)
        internal
        view
        returns (uint256 _tokenId)
    {
    ...
    function _royaltyInfo(uint256 _tokenId, uint256 _value)
        internal
        view
        returns (address _receiver, uint256 _royaltyAmount)
    {
        ...
        if (
            ...
            royaltiesRegistryProxy.hasRoyalties(editionId)
        ) {
            // Note: any registry must be edition aware so to only store one entry for all within the edition
            (_receiver, _royaltyAmount) = royaltiesRegistryProxy.royaltyInfo(
                editionId,
                _value
            );
        } else {
    ...
    {
        address originalCreator = _getCreatorOfEdition(
            _editionFromTokenId(_tokenId)
        );
        (address _receiver, uint256 _royaltyAmount) = _royaltyInfo(
            _tokenId,
            _value
        );
    ...
    function hasRoyalties(uint256 _editionId)
        external
        view
        override
        validateEdition(_editionId)
        returns (bool)
    {
        return
            (royaltyRegistryActive() &&
                royaltiesRegistryProxy.hasRoyalties(_editionId)) ||
            secondarySaleRoyalty > 0;
    }

    function getRoyaltiesReceiver(uint256 _tokenId)
        public
        view
        override
        returns (address)
        ...
        returns (uint256[] memory)
    {
        ...
        feeBps[0] = uint256(secondarySaleRoyalty) / basisPointsModulo;
    ...
    function getAllUnsoldTokenIdsForEdition(uint256 _editionId)
        public
        view
        validateEdition(_editionId)
        returns (uint256[] memory)
        ...
        )
    {
        ...
        if (
            ...
            address _receiver = royaltiesRegistryProxy.getRoyaltiesReceiver(
                _editionId
            );
    ...
    function isEditionSoldOut(uint256 _editionId)
        public
        view
        override
        validateEdition(_editionId)
        ...
        returns (address)
    {
        ...
        if (
            possibleCreator != address(0) &&
            (_maxTokenIdOfEdition(_editionId) - 1) >= _tokenId
        ) {
    ...
    /// @dev Allowing for batch transfers from the provided address, will fail if from does not own all the tokens
    function consecutiveBatchTransferFrom(
        address _from,
        address _to,
    ...
    function setRoyaltiesRegistryProxy(IERC2981 _royaltiesRegistryProxy)
        public
        onlyAdmin
    {
        royaltiesRegistryProxy = _royaltiesRegistryProxy;
    ...
    function setTokenUriResolver(ITokenUriResolver _tokenUriResolver)
        public
        onlyAdmin
    {
    ...
    function composeERC20sAsCreator(
        uint16 _editionId,
        address[] calldata _erc20s,
        uint256[] calldata _amounts
    ) external validateCreator(_editionId) {
    ...
    function lockInAdditionalMetaData(
        uint256 _editionId,
        string calldata _metadata
    ) external validateCreator(_editionId) {
        require(
            bytes(sealedEditionMetaData[_editionId]).length == 0,
            "Already set"
        );
    ...
    function lockInAdditionalTokenMetaData(
        uint256 _tokenId,
        string calldata _metadata
    ) external {
        require(
        ...
        require(
            bytes(sealedTokenMetaData[_tokenId]).length == 0,
            "Already set"
        );
